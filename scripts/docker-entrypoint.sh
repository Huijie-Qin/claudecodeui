...
HOME="${HOME:-/home/cloudcli}"
DATA_ROOT="${CLOUDCLI_DATA_ROOT:-$HOME/.cloudcli}"
DATABASE_PATH="${DATABASE_PATH:-${CLOUDCLI_DATABASE_PATH:-$DATA_ROOT/auth.db}}"
WORKSPACES_ROOT="${WORKSPACES_ROOT:-${CLOUDCLI_WORKSPACES_ROOT:-$HOME}}"
CLOUDCLI_RUNTIME_ROOT="${CLOUDCLI_RUNTIME_ROOT:-${CLOUDCLI_RUNTIME_PATH:-$DATA_ROOT/runtimes}}"
CLOUDCLI_HOOK_SKILLS_ROOT="${CLOUDCLI_HOOK_SKILLS_ROOT:-$DATA_ROOT/hook-skills}"
CLOUDCLI_DATA_ROOT="$DATA_ROOT"
export CLOUDCLI_DATA_ROOT CLOUDCLI_HOOK_SKILLS_ROOT CLOUDCLI_RUNTIME_ROOT DATABASE_PATH HOME WORKSPACES_ROOT

case "$CLOUDCLI_HOOK_SKILLS_ROOT" in
  /)
    echo "CLOUDCLI_HOOK_SKILLS_ROOT must not be the filesystem root" >&2
    exit 1
    ;;
  /*) ;;
  *)
    echo "CLOUDCLI_HOOK_SKILLS_ROOT must be an absolute path" >&2
    exit 1
    ;;
esac
...
exec gosu "$APP_USER" env \
  CLOUDCLI_DATA_ROOT="$CLOUDCLI_DATA_ROOT" \
  CLOUDCLI_HOOK_SKILLS_ROOT="$CLOUDCLI_HOOK_SKILLS_ROOT" \
  CLOUDCLI_RUNTIME_ROOT="$CLOUDCLI_RUNTIME_ROOT" \
