...
function parseUrl(value: string, key: DesktopEnvKey, production: boolean): URL {
  let parsed: URL;
  try {
  ...
  const developmentLoopback = !production
    && parsed.protocol === 'http:'
    && LOOPBACK_HOSTS.has(parsed.hostname);
  if (!secure && !developmentLoopback) {
    throw new Error(`${key} must use HTTPS${production ? '' : ' (or loopback HTTP in development)'}.`);
  }
...
function parseOriginList(
  ...
  const origins = value.split(',').map((entry) => {
    const parsed = parseUrl(entry.trim(), key, production);
...
export function createDesktopBuildConfig(
  ...
  const homeUrl = parseUrl(homeValue, 'DESKTOP_HOME_URL', options.production);
  if (homeUrl.search || homeUrl.hash) {
    throw new Error('DESKTOP_HOME_URL must not contain a query or fragment.');
  }

  const updateUrl = parseUrl(updateValue, 'DESKTOP_UPDATE_BASE_URL', options.production);
