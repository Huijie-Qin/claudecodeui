...
4. `PostToolUse` Hook 将任务交给与主代理相同的持久化 MCP Loop 调度器；调度器每 10 秒使用相同任务 ID 查询状态，并执行 Python 终止判断。
...
子代理循环作业已持久化，并与主代理共享并发、重试、脚本判断和逐轮审计；但原生 Hook 回调及子代理上下文仍驻留 SDK 进程，因此服务重启后无法把完成结果送回原子代理。最终成功运行期间没有重启服务。多个并行长循环、后台子代理和 40 分钟真实运行不属于本轮覆盖范围。当前 MCP 不使用鉴权 helper；现有 `headersHelper` 的准备耗时不计入单次 MCP 请求超时，循环的总截止时间仍独立生效。
