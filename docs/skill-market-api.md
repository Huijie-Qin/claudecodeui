...
CCUI normalizes trailing slashes in `SKILL_MARKET_BASE_URL` to one slash, then directly appends the remote Skill endpoints under `/data-agent/api/skill/...`. `http://` and `https://` are both supported.
...
- If an imported skill no longer exists in the remote list/search result, CCUI marks it as `remoteDeleted`; the local Files copy remains readable and can be uploaded/published again as a new remote skill.
