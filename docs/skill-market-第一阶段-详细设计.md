...
普通用户编辑器插入为一次性字符串替换：捕获文件路径、内容摘要、selectionStart/End；弹框搜索不丢焦点；选择预览；提交前核对编辑目标未改变，再替换选区并交还焦点。前端可保留一次撤销操作。正文插入不触发保存、工具启用或 SQL Check 执行。第一阶段不出现其他 Skill/MCP 选择 tab。
...
1. 冻结全部用例与输入，记录 baseContentHash；前测不可跳过。
...
4. 先在临时目录生成并校验；锁内再次比对 baseContentHash。仅哈希相同才提交；外部编辑则 stale，不替用户做盲目覆盖。
...
- error/inconclusive、权限撤销、用户停止、并发冲突、总时长/费用超限均终止循环，单列原因；不能当成普通 failed 持续优化。保留已提交改动和已完成证据，不自动回滚或发布。
- 任务保存 `maxIterations, iteration, stopReason, rounds[]`；每轮记录固定预期引用、输入文件摘要、优化前后文件摘要、完整逐例输出、写入状态和耗时。重启恢复须在同一轮日志上继续，防止重复写入。
- 页面显示“已执行 X / N 次迭代”和终止原因；用例详情默认对比初始前测与最新一轮后测，明确标注轮次。文件对比展示本任务初始文件与最终已落盘文件，不称版本历史。运行中不能修改上限。
