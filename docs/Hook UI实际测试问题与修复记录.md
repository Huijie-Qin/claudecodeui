...
第一次实现把“是否在会话展示”放进了 Admin 的 Hook 配置，形成了所有用户共用的全局开关。这与需求语义不符：展示方式应由用户在“设置 → 辅助功能”中自行选择，不能由管理员修改 Hook 定义后统一影响所有人。历史审计恢复如果仍读取全局字段，也会导致不同用户无法拥有独立展示偏好。

### 修复方案

- Admin Hook 编辑器移除“在对话中展示 Hook”，Hook 定义的创建、更新和发布接口不再读写展示字段。
- 新增 `user_hook_preferences`，使用 `(user_id, hook_id)` 保存每个用户对每个 Hook 的 `show_in_chat` 偏好；设置与 Hook 启用绑定相互独立，停用再启用后仍保留原展示选择。
- “设置 → 辅助功能”的每个 Hook 增加带明确标签的“对话展示”开关，包括启用状态由 SQL Check 专页管理的强制校验 Hook。页面明确提示该选项只影响当前用户，关闭后 Hook 仍执行且执行记录仍可查看。
- 开关关闭时，当前用户的实时 `hook_activity` 和后置 Skill 过程消息不进入对话展示；Hook 脚本、MCP、数据记录、Skill/Agent 后置行为和执行审计继续运行。
- 后置 Skill 回合的思考、工具调用、工具结果和最终文本统一携带对应的 Hook 活动 ID；历史加载时也会根据 Claude 内部回合标记恢复这个关联，直到下一个真实用户回合为止。
- 历史会话按当前登录用户的偏好同时过滤执行审计卡片、已持久化的后置活动以及关联的 Skill 过程消息；同一 Hook 可对甲用户隐藏、对乙用户显示。
...
- 数据迁移会把旧全局关闭值转换为当时已绑定用户的个人关闭偏好，再把旧列归一为开启；以后新用户不会继承管理员曾经设置的全局关闭状态，用户已经手动更新的偏好也不会被后续启动覆盖。
- 兼容缺少用户偏好数据的旧数据库：默认展示；偏好查询失败时保留既有活动，不让旧会话历史整体加载失败。
- 展示开启时，同一次执行的后置行为及其完整 Skill 过程都收纳在主 Hook 卡片内；默认只显示“后置行为 N”，用户点击后才渲染 Skill、参数、状态、思考、工具调用和最终结果。

### 真实 UI 与运行时验证

使用重新构建且健康检查通过的生产 Docker 页面完成了实际验证：

1. 打开 Admin 的“对话正常结束通知”编辑器，精确文本“在对话中展示 Hook”和对应 switch 的 DOM 数量均为 `0`。
2. 打开当前 root 用户的“设置 → 辅助功能”，5 个可用 Hook 均显示“对话展示”开关；SQL Check 强制校验的“启用”开关保持禁用并注明由专页管理，但其用户级“对话展示”开关可操作。
3. 关闭“SQL 行数记录”的对话展示前，当前历史会话中该 Hook 紫色卡片为 `9` 张；关闭后开关的 `aria-checked=false`。
4. 保持展示关闭时打开“我的执行记录”，仍能看到“共 9 次执行”和 `7` 条旧版独立数据记录，证明关闭展示没有删除或停止审计记录。
5. 刷新会话后，“SQL 行数记录”紫色卡片由 `9` 张变为 `0`；同页“SQL Check 强制校验”仍有 `11` 张，证明只隐藏了用户选择的单个 Hook，没有误伤其他 Hook。
6. 恢复“SQL 行数记录”的展示并再次刷新，开关回到 `aria-checked=true`，对应紫色卡片恢复为 `9` 张。测试结束时用户偏好已恢复原值。

### 自动化回归

| 验证项 | 结果 |
| --- | --- |
| 完整 Hook 服务、运行时与 28 事件矩阵回归 | 78/78 通过 |
| Hook 配置与用户展示偏好服务测试 | 22/22 通过 |
| Claude 会话历史恢复测试 | 17/17 通过 |
| Workspace Hook 路由测试 | 21/21 通过 |
| Admin Hook 草稿与目录前端测试 | 11/11 通过 |
| 客户端与服务端 TypeScript | 全部通过 |
