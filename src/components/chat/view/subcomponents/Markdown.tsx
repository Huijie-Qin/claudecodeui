...
import { resolveFilePreviewLink } from '../../../file-preview/filePreviewLinks';
...
export function Markdown({ children, className, onFileOpen }: MarkdownProps) {
  ...
  const components = useMemo(() => ({
    ...
    a: ({ href, children: linkChildren }: { href?: string; children?: React.ReactNode }) => {
      const filePath = onFileOpen ? resolveFilePreviewLink(href) : null;
      return (
        <a
          href={href}
          ...
          onClick={(event) => {
            if (!filePath || !onFileOpen || event.defaultPrevented || event.button !== 0
